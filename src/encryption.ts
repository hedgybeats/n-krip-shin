...
const encryptFileStream = async (algorithm: string, filePath: string, attempt = 0, outFilePath?: string, overwrite = false): Promise<Encryptionresult> => {
  ...
  const res = await new Promise<Encryptionresult | NodeJS.ErrnoException>((resolve) => {
    pipeline(createReadStream(filePath), createCipheriv(algorithm, Buffer.from(key), iv), createWriteStream(outFilePath, overwrite ? undefined : {flags: 'wx'}))
      .then(() =>
        resolve({
...
const decryptFileStream = async (algorithm: string, key: string, iv: string, filePath: string, attempt = 0, outFilePath?: string, overwrite = false): Promise<Decryptionresult> => {
  if (outFilePath == undefined) {
  ...
  const res = await new Promise<Decryptionresult | NodeJS.ErrnoException>((resolve) => {
    pipeline(createReadStream(filePath), createDecipheriv(algorithm, Buffer.from(key, 'hex'), Buffer.from(iv, 'hex')), createWriteStream(outFilePath, overwrite ? undefined : {flags: 'wx'}))
      .then(() =>
        resolve({
          filePath: outFilePath,
          duration: 0,
